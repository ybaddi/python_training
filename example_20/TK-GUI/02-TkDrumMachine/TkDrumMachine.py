...
class DrumMachine():
    ...
    def save_project(self):
        ...
        file_name = filedialog.asksaveasfilename(filetypes=[('Drum Beat File','*.bt')], title="Save project as...")
        pickle.dump(self.pattern_list, open(file_name, "wb"))
        self.root.title(os.path.basename(file_name) + " - Drum Beast")

    def load_project(self):
        file_name = filedialog.askopenfilename(filetypes=[('Drum Beat File','*.bt')], title="Load Project")
        if file_name == '' :return
        self.root.title(os.path.basename(file_name) + " - Drum Beast")
        ...
        try:
            ...
            messagebox.showerror("Error", "An unexpected error occurred trying to reconstruct patterns")
